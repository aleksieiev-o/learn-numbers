...
const initialValues: ISignUpDto = {
  email: '',
  password: '',
  confirmPassword: '',
  displayName: '',
...
const SignUp: FC<Props> = (props): ReactElement => {
  const {isLoading, handleActionModalButton, handleCloseModalButton, setAuthModalType} = props;
  const { t } = useTranslation(['common', 'auth']);

  const submitHandler = async (payload: ISignUpDto, formikHelpers: FormikHelpers<ISignUpDto>) => {
    try {
  ...
  return (
    <ModalContent>
      ...
      <ModalHeader>{t('auth_modal_sign_up_title', {ns: 'auth'})}</ModalHeader>
      ...
      <ModalBody>
        <Stack>
          ...
          <Text>{t('auth_modal_sign_up_desc', {ns: 'auth'})}</Text>
        </Stack>
      </ModalBody>
      ...
      <ModalFooter>
        ...
        <Button
          ...
          isLoading={isLoading}
          colorScheme={'telegram'}
          title={t('auth_modal_sign_up_btn_text', {ns: 'auth'})!}>
